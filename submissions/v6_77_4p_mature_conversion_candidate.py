...
MATURE_CONVERSION_STEP = 95
MATURE_CONVERSION_MIN_PROD = 45
MATURE_CONVERSION_MIN_STRENGTH_RATIO = 0.82
MATURE_REAR_SEND_RATIO = 0.40
MATURE_REAR_SOURCE_MIN = 22
MATURE_REAR_SEND_MIN = 18
MATURE_PRESSURE_MIN_LEFT = 78
MATURE_PRESSURE_MIN_SEND = 72
MATURE_PRESSURE_MAX_SEND = 380
MATURE_PRESSURE_BASE_RATIO = 0.92
MATURE_PRESSURE_MAX_PER_TURN = 4
MATURE_PRESSURE_MAX_TURNS = 48
MATURE_PRESSURE_TARGET_MIN_PROD = 1
MATURE_PRESSURE_SCORE_MULT = 1.42
MATURE_PRESSURE_LOW_PROD_MULT = 0.92
...
def mature_conversion_active(world, modes=None):
    if (
        not world.is_four_player
        or world.is_late
        or world.step < MATURE_CONVERSION_STEP
        or world.my_prod < MATURE_CONVERSION_MIN_PROD
        or not world.enemy_planets
        or world.max_enemy_strength <= 0
    ):
        return False

    if (
        world.my_total < world.max_enemy_strength * MATURE_CONVERSION_MIN_STRENGTH_RATIO
        and world.my_prod < world.max_enemy_prod + 4
    ):
        return False
    ...
    if (
        ...
        and world.my_prod + 8 < world.max_enemy_prod
    ):
        return False
...
def build_pressure_packet_missions(world, policy, planned_commitments, modes):
    if not PRESSURE_PACKET_ENABLED:
        return []
    profile = strategy_profile(world)
    mature_conversion = mature_conversion_active(world, modes)
    if world.step < PRESSURE_PACKET_TURN or world.remaining_steps < PRESSURE_PACKET_STOP_REMAINING:
        return []
    if world.my_prod < (
        MATURE_CONVERSION_MIN_PROD if mature_conversion else profile["pressure_packet_min_my_prod"]
    ):
        return []
    if not world.enemy_planets:
        return []

    target_min_prod = MATURE_PRESSURE_TARGET_MIN_PROD if mature_conversion else 2
    missions = []
    target_pool = [
        target
        for target in world.enemy_planets
        if target.id not in world.comet_ids and target.production >= target_min_prod
    ]
    if not target_pool:
        return missions

    min_left = MATURE_PRESSURE_MIN_LEFT if mature_conversion else PRESSURE_PACKET_MIN_LEFT
    min_send = MATURE_PRESSURE_MIN_SEND if mature_conversion else profile["pressure_packet_min_send"]
    max_send = MATURE_PRESSURE_MAX_SEND if mature_conversion else PRESSURE_PACKET_MAX_SEND
    max_turns = MATURE_PRESSURE_MAX_TURNS if mature_conversion else PRESSURE_PACKET_MAX_TURNS
    base_ratio = MATURE_PRESSURE_BASE_RATIO if mature_conversion else profile["pressure_packet_base_ratio"]
    ...
    for src in world.my_planets:
        ...
        ratio = base_ratio
        if not mature_conversion and (
            modes.get("is_behind") or modes.get("is_four_player_leader_pressure")
        ):
        ...
        for target in target_pool:
            ...
            if mature_conversion:
                value *= MATURE_PRESSURE_SCORE_MULT
                if target.production <= 1:
                    value *= MATURE_PRESSURE_LOW_PROD_MULT
            ...
            option = ShotOption(
                ...
                needed=need,
                send_cap=desired,
                mission="mature_pressure_packet" if mature_conversion else "pressure_packet",
            )
            mission = Mission(
                kind="mature_pressure_packet" if mature_conversion else "pressure_packet",
                score=score,
                target_id=target.id,
                turns=turns,
                options=[option],
...
def plan_moves(world, deadline=None):
    ...
    for mission in missions:
        ...
        if mission.kind in ("pressure_packet", "mature_pressure_packet"):
            mature_packet = mission.kind == "mature_pressure_packet"
            max_packets = (
                MATURE_PRESSURE_MAX_PER_TURN if mature_packet else profile["pressure_packet_max_per_turn"]
            )
            min_send = MATURE_PRESSURE_MIN_SEND if mature_packet else profile["pressure_packet_min_send"]
            max_turns = MATURE_PRESSURE_MAX_TURNS if mature_packet else PRESSURE_PACKET_MAX_TURNS
    ...
    if (
        ...
        if safe_fronts:
            ...
            if mature_conversion:
                send_ratio = min(send_ratio, MATURE_REAR_SEND_RATIO)
                rear_source_min = max(rear_source_min, MATURE_REAR_SOURCE_MIN)
                rear_send_min = max(rear_send_min, MATURE_REAR_SEND_MIN)
