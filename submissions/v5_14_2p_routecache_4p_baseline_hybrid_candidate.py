...
class WorldModel:
    ...
    def target_position_at(self, target, turns):
        turn = int(turns)
        key = (target.id, turn)
        cached = self.target_position_cache.get(key)
        if cached is not None:
            return cached
        pos = predict_target_position(
            target, turn, self.initial_by_id, self.ang_vel, self.comets, self.comet_ids,
        )
        if pos is not None:
            pos = (float(pos[0]), float(pos[1]))
        self.target_position_cache[key] = pos
        return pos

    def estimate_arrival_to(self, src, target, ships, position_key, tx, ty):
        key = (src.id, target.id, int(ships), position_key)
        cached = self.estimate_cache.get(key)
        if cached is not None or key in self.estimate_cache:
            return cached
        result = estimate_arrival(src.x, src.y, src.radius, tx, ty, target.radius, ships)
        self.estimate_cache[key] = result
        return result

    def search_safe_intercept(self, src, target, ships):
        best = None
        best_score = None
        max_turns = min(HORIZON, ROUTE_SEARCH_HORIZON)
        if target.id in self.comet_ids:
            max_turns = min(max_turns, max(0, comet_remaining_life(target.id, self.comets) - 1))

        for candidate_turns in range(1, max_turns + 1):
            pos = self.target_position_at(target, candidate_turns)
            if pos is None:
                continue
            est = self.estimate_arrival_to(src, target, ships, candidate_turns, pos[0], pos[1])
            if est is None:
                continue
            _, turns = est
            if abs(turns - candidate_turns) > INTERCEPT_TOLERANCE:
            ...
            actual_turns = max(turns, candidate_turns)
            actual_pos = self.target_position_at(target, actual_turns)
            if actual_pos is None:
                continue

            confirm = self.estimate_arrival_to(
                src, target, ships, actual_turns, actual_pos[0], actual_pos[1],
            )
            if confirm is None:
                continue
    ...
    def aim_with_prediction(self, src, target, ships):
        est = self.estimate_arrival_to(src, target, ships, 0, target.x, target.y)
        if est is None:
            if not target_can_move(target, self.initial_by_id, self.comet_ids):
                return None
            return self.search_safe_intercept(src, target, ships)

        tx, ty = target.x, target.y
        for _ in range(5):
            _, turns = est
            pos = self.target_position_at(target, turns)
            if pos is None:
                return None
            ntx, nty = pos
            next_est = self.estimate_arrival_to(src, target, ships, int(turns), ntx, nty)
            if next_est is None:
                if not target_can_move(target, self.initial_by_id, self.comet_ids):
                    return None
                return self.search_safe_intercept(src, target, ships)
        ...
        final_est = self.estimate_arrival_to(src, target, ships, ("final", round(tx, 3), round(ty, 3)), tx, ty)
        if final_est is None:
            return self.search_safe_intercept(src, target, ships)
        return final_est[0], final_est[1], tx, ty
    ...
    def plan_shot(self, src_id, target_id, ships):
        ...
        if self.is_four_player:
            ...
            result = self.aim_with_prediction(src, target, ships)
...
def plan_moves(world, deadline=None):
    ...
    if world.is_four_player:
        ...
        missions.sort(key=lambda item: (mission_priority.get(item.kind, 2), -item.score))
