...
FOUR_PLAYER_FAST_OPENING_TURN = 16
FOUR_PLAYER_FAST_OPENING_MIN_PROD = 3
FOUR_PLAYER_FAST_OPENING_MAX_SHIPS = 16
FOUR_PLAYER_FAST_OPENING_MAX_TURNS = 18
FOUR_PLAYER_FAST_OPENING_SAVE_SHIPS = 1
...
def fast_four_player_opening_capture(target, arrival_turns, needed, src_available, world, policy):
    if (
        not world.is_four_player
        or world.step >= FOUR_PLAYER_FAST_OPENING_TURN
        or target.owner != -1
        or target.id in world.comet_ids
        or target.production < FOUR_PLAYER_FAST_OPENING_MIN_PROD
        or int(target.ships) > FOUR_PLAYER_FAST_OPENING_MAX_SHIPS
        or arrival_turns > FOUR_PLAYER_FAST_OPENING_MAX_TURNS
        or needed > src_available - FOUR_PLAYER_FAST_OPENING_SAVE_SHIPS
    ):
        return False

    my_t, enemy_t = policy_reaction_times(target.id, policy)
    return enemy_t >= my_t - FOUR_PLAYER_ROTATING_REACTION_GAP
...
def opening_filter(target, arrival_turns, needed, src_available, world, policy, src=None):
    ...
    if world.is_four_player:
        if fast_four_player_opening_capture(
            target, arrival_turns, needed, src_available, world, policy,
        ):
            return False
...
def preferred_send(target, base_needed, arrival_turns, src_available, world, modes, policy):
    ...
    if fast_four_player_opening_capture(
        target, arrival_turns, base_needed, src_available, world, policy,
    ):
        saved_cap = max(base_needed, src_available - FOUR_PLAYER_FAST_OPENING_SAVE_SHIPS)
        return min(saved_cap, base_needed + 1)
