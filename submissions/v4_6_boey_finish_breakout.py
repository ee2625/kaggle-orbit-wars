...
TWO_PLAYER_FINISH_START_STEP = 55
...
def _agent_impl(obs: Any) -> List[List[float]]:
    ...
    player_count = game_player_count(obs, planets, fleets, player)
...
def choose_target(
    ...
    finish_pressure = is_two_player_finish_pressure(
        all_planets,
        fleets,
        comet_ids,
        player,
    ...
    for target in targets:
        if finish_pressure and target.owner == -1:
            if target.id in comet_ids or target.production <= 2:
                continue
        ...
        if (
            target.owner != -1
            and not leader_pressure
            and not finish_pressure
            and should_delay_enemy_attack(step, my_planet_count, owned_ratio, has_affordable_neutral)
        ):
            continue
        ...
        min_ships_for_target = (
            1 if player_count <= 2 and target.owner == -1 and target.ships <= 3 else MIN_ATTACK_SHIPS
        )
        ...
        if arrival_needed > ships and arrival_needed <= available:
            ships = ships_to_send(arrival_needed, available, safety_factor, min_ships_for_target)
            aim = aim_target_position(target, source, ships, obs, angular_velocity, comet_ids)
            if aim is None:
                continue
            tx, ty = aim
        ...
        if leader_pressure:
            score += 150.0
        if finish_pressure and target.owner not in (-1, player):
            score += 180.0
...
def should_save_for_high_value_opening(
    ...
    if should_save_for_low_production_breakout(
        source,
        target,
        targets,
        all_planets,
    ...
    if (
        player_count >= 4
        and source.production >= 4
        and target.production >= 2
        and target_distance <= 26.0
    ):
        return False
...
def should_save_for_low_production_breakout(
    source: Planet,
    target: Planet,
    targets: Sequence[Planet],
    all_planets: Sequence[Planet],
    ...
    if player_count != 2 or step >= 34:
        return False
    if source.production > 1 or target.owner != -1 or target.production > 1 or target.id in comet_ids:
        return False

    low_value_claims = sum(
        1
        for planet in all_planets
        if planet.id not in comet_ids and planet.production <= 1 and planet.owner == player
    )
    for planet in targets:
        if planet.id in comet_ids or planet.owner != -1 or planet.production > 1:
            continue
        friendly_incoming = incoming.get(planet.id, {}).get(player, 0) + planned_by_target.get(planet.id, 0)
        if friendly_incoming > 0:
            low_value_claims += 1

    if low_value_claims < 2:
        return False

    future_available = available + source.production * 20
    for candidate in targets:
        if candidate.id == target.id or candidate.owner != -1 or candidate.production < 3 or candidate.id in comet_ids:
            continue
...
def ships_to_send(
    needed: int,
    available: int,
    safety_factor: float = CAPTURE_SAFETY_FACTOR,
    min_ships: int = MIN_ATTACK_SHIPS,
) -> int:
    if needed > available:
        return 0

    padded = max(needed, int(math.ceil(needed * safety_factor)), min_ships)
...
def two_player_finish_pressure(
    moves: List[List[float]],
    my_planets: Sequence[Planet],
    all_planets: Sequence[Planet],
    incoming: dict[int, dict[int, int]],
    ...
    if step < TWO_PLAYER_FINISH_START_STEP or my_planet_count < 4:
        return
    ...
    if not should_enter_two_player_finish_mode(durable_planets, enemy_planets, all_planets, fleets, comet_ids, player):
        return

    allow_siege = should_siege_remaining_enemy(enemy_planets, all_planets, fleets, comet_ids, player)
    ships_sent: dict[int, int] = {}
    planned_by_target: dict[int, int] = {}
    for move in moves:
        source_id = int(move[0])
        target = projected_move_target(move, all_planets, obs, angular_velocity, comet_ids)
        ships = int(move[2])
        ships_sent[source_id] = ships_sent.get(source_id, 0) + ships
        if target is not None:
            planned_by_target[target.id] = planned_by_target.get(target.id, 0) + ships

    max_extra_attacks = 3
    extra_attacks = 0
    for source in sorted(durable_planets, key=lambda planet: (planet.ships, planet.production), reverse=True):
        if extra_attacks >= max_extra_attacks:
            return

        sent = ships_sent.get(source.id, 0)
        reserve = reserve_for(source, step, my_planet_count) + threatened_reserve(source, incoming, player)
        available = int(source.ships - sent - reserve)
        if available < MIN_ATTACK_SHIPS:
            continue

        choice = choose_finish_attack(
            source,
            enemy_planets,
            all_planets,
            incoming,
...
def should_enter_two_player_finish_mode(
    my_planets: Sequence[Planet],
    ...
    production = owner_production(all_planets, comet_ids)
    my_prod = production.get(player, 0)
    enemy_prod = max((prod for owner, prod in production.items() if owner != player and owner >= 0), default=0)
    if my_prod >= enemy_prod + max(4, int(enemy_prod * 0.18)):
        return True

    if len(my_planets) >= len(enemy_planets) + 4 and my_prod >= enemy_prod:
        return True

    scores = owner_ship_scores(all_planets, fleets)
    my_score = scores.get(player, 0)
    enemy_score = max((score for owner, score in scores.items() if owner != player and owner >= 0), default=0)
    return my_score >= enemy_score + max(120, int(enemy_score * 0.35)) and my_prod >= enemy_prod - 2


def is_two_player_finish_pressure(
    all_planets: Sequence[Planet],
    fleets: Sequence[Fleet],
    comet_ids: set,
    player: int,
    ...
    if player_count != 2 or step < TWO_PLAYER_FINISH_START_STEP:
        return False

    my_planets = [
        planet for planet in all_planets
        if planet.owner == player and planet.id not in comet_ids
    ]
    enemy_planets = [
        planet for planet in all_planets
        if planet.owner not in (-1, player) and planet.id not in comet_ids
    ]
    if len(my_planets) < 4 or not enemy_planets:
        return False

    return should_enter_two_player_finish_mode(my_planets, enemy_planets, all_planets, fleets, comet_ids, player)


def should_siege_remaining_enemy(
    enemy_planets: Sequence[Planet],
    all_planets: Sequence[Planet],
    fleets: Sequence[Fleet],
    comet_ids: set,
    ...
    if len(enemy_planets) > 5:
        return False

    production = owner_production(all_planets, comet_ids)
    my_prod = production.get(player, 0)
    enemy_prod = max((prod for owner, prod in production.items() if owner != player and owner >= 0), default=0)
    if my_prod >= enemy_prod + max(8, int(enemy_prod * 0.5)):
        return True

    scores = owner_ship_scores(all_planets, fleets)
    my_score = scores.get(player, 0)
    enemy_score = max((score for owner, score in scores.items() if owner != player and owner >= 0), default=0)
    return my_score >= enemy_score + max(250, int(enemy_score * 0.8)) and my_prod >= enemy_prod


def choose_finish_attack(
    source: Planet,
    enemy_planets: Sequence[Planet],
    all_planets: Sequence[Planet],
    incoming: dict[int, dict[int, int]],
    ...
    for target in enemy_planets:
        ...
        ships = ships_to_send(needed, available, safety_factor=1.06)
        partial_siege = False
        if ships <= 0 and allow_siege and available >= MIN_ATTACK_SHIPS:
            ships = max(MIN_ATTACK_SHIPS, int(available * 0.72))
            partial_siege = True
        if ships <= 0:
            continue
        ...
        eta = distance / fleet_speed(ships)
        arrival_needed = needed_after_travel(target, needed, eta)
        if arrival_needed > ships and not partial_siege:
            ships = ships_to_send(arrival_needed, available, safety_factor=1.06)
            if ships <= 0:
                continue
            aim = aim_target_position(target, source, ships, obs, angular_velocity, comet_ids)
            if aim is None:
        ...
        score = target.production * 42.0 - arrival_needed * 0.55 - eta * 0.7 - distance * 0.12
        if partial_siege:
            score += target.production * 28.0 + ships * 0.18
        if target.ships <= max(ships * 0.65, target.production * 4):
            score += 30.0
        if best is None or score > best[0]:
            best = (score, target, angle, ships)
...
def projected_move_target(
    ...
    angle = float(move[1])
    ships = int(move[2])
    source = next((planet for planet in planets if planet.id == source_id), None)
    if source is None:
        return None
    ...
    return projected_fleet_target(fleet, planets, obs, angular_velocity, comet_ids, max_turns=140)
...
def forward_rebalance(
    ...
    if should_prioritize_leader_pressure_over_rebalance(all_planets, comet_ids, player, step):
        return
...
def should_prioritize_leader_pressure_over_rebalance(
    all_planets: Sequence[Planet],
    comet_ids: set,
    player: int,
    step: int,
    ...
    enemy_best = max(
        (prod for owner, prod in production.items() if owner != player and owner >= 0),
        default=0,
    )
    return enemy_best - my_prod >= max(3, int(my_prod * 0.2))
...
def game_player_count(obs: Any, planets: Sequence[Planet], fleets: Sequence[Fleet], player: int) -> int:
    initial_owners: set[int] = set()
    for row in get_field(obs, "initial_planets", []) or []:
        try:
            owner = int(row[1])
        except (TypeError, ValueError, IndexError):
            continue
        if owner >= 0:
            initial_owners.add(owner)
    if initial_owners:
        return len(initial_owners)

    owners = {player}
    for planet in planets:
        if planet.owner >= 0:
            owners.add(int(planet.owner))
    for fleet in fleets:
        if fleet.owner >= 0:
            owners.add(int(fleet.owner))
    return len(owners)
