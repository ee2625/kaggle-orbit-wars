...
FOUR_PLAYER_HIGH_PROD_NEUTRAL_VALUE_MULT = 1.08
FOUR_PLAYER_CONTESTED_HIGH_PROD_NEUTRAL_VALUE_MULT = 0.92
FOUR_PLAYER_LOW_PROD_OPENING_VALUE_MULT = 0.86
...
REAR_SEND_RATIO_FOUR_PLAYER = 0.62
REAR_SEND_MIN_SHIPS = 10
REAR_MAX_TRAVEL_TURNS = 40

PARTIAL_SOURCE_MIN_SHIPS = 4
...
def opening_filter(target, arrival_turns, needed, src_available, world, policy, src=None):
    ...
    if target.id in world.comet_ids:
        return False
    initial_src = world.initial_by_id.get(src.id) if src is not None else None
    if (
        world.is_four_player
        and src is not None
        and initial_src is not None
        and initial_src.owner != world.player
        and world.step < 42
        and target.production >= 5
        and not world.is_static(target.id)
        and not is_safe_neutral(target, policy)
    ):
        return True
    if (
        src is not None
    ...
    if (
        ...
        for candidate in world.neutral_planets:
            ...
            if aim[1] > 30:
                continue
            my_t, enemy_t = policy_reaction_times(candidate.id, policy)
            if enemy_t >= my_t + SAFE_NEUTRAL_MARGIN:
                return True
        ...
        if world.is_four_player and world.is_opening:
            if target.production >= 4:
                if is_safe_neutral(target, policy):
                    ...
                    value *= FOUR_PLAYER_CONTESTED_HIGH_PROD_NEUTRAL_VALUE_MULT
            elif target.production <= 1 and any(
                planet.production >= 4 and planet.id not in world.comet_ids
