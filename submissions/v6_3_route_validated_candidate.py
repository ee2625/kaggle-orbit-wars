...
class WorldModel:
    ...
    def plan_shot(self, src_id, target_id, ships):
        ...
        if result is not None:
            ...
            validate_route = (
                self.is_opening
                or self.is_four_player
                or target_can_move(target, self.initial_by_id, self.comet_ids)
                or target.id in self.comet_ids
            )
            if validate_route:
                max_turns = max(4, int(turns) + 3)
                if self.is_four_player:
                    max_turns = min(max_turns, FOUR_PLAYER_ROUTE_VALIDATE_TURN_LIMIT)
                if not shot_hits_intended_target(
                    src,
                    target,
                    angle,
                    ships,
