...
FOUR_PLAYER_CONVERSION_ENEMY_MULT = 1.55
FOUR_PLAYER_CONVERSION_LEADER_MULT = 1.55
FOUR_PLAYER_CONVERSION_LOW_NEUTRAL_MULT = 0.38
FOUR_PLAYER_CONVERSION_MID_NEUTRAL_MULT = 0.72
FOUR_PLAYER_PRESSURE_PACKET_FRACTION = 0.60
FOUR_PLAYER_PRESSURE_PACKET_MIN = 44
FOUR_PLAYER_FUNNEL_STARVED_RATIO = 0.84
FOUR_PLAYER_FUNNEL_CONVERSION_RATIO = 0.76
...
def plan_moves(world, deadline=None):
    ...
    if (
        ...
        for src in sorted(world.my_planets, key=lambda planet: -source_attack_left(planet.id)):
            ...
            best = None
            for target in pressure_targets[:10]:
                if expired():
                    return finalize_moves()
                if target.id == src.id:
                    continue
