...
FOUR_PLAYER_CONVERSION_TURN = 28
FOUR_PLAYER_CONVERSION_END_TURN = 150
FOUR_PLAYER_CONVERSION_PROD_TARGET = 56
FOUR_PLAYER_STARVED_PROD_TARGET = 34
FOUR_PLAYER_CONVERSION_ENEMY_MULT = 1.42
FOUR_PLAYER_CONVERSION_LEADER_MULT = 1.50
FOUR_PLAYER_CONVERSION_LOW_NEUTRAL_MULT = 0.45
FOUR_PLAYER_CONVERSION_MID_NEUTRAL_MULT = 0.78
FOUR_PLAYER_PRESSURE_PACKET_FRACTION = 0.56
FOUR_PLAYER_PRESSURE_PACKET_MIN = 40
FOUR_PLAYER_FUNNEL_STARVED_RATIO = 0.80
FOUR_PLAYER_FUNNEL_CONVERSION_RATIO = 0.74
...
def build_modes(world):
    ...
    is_four_player_starved = (
        world.is_four_player
        and FOUR_PLAYER_CONVERSION_TURN <= world.step <= FOUR_PLAYER_CONVERSION_END_TURN
        and not is_ahead
        and four_player_conversion_profile(world)
        and (
            world.my_prod < FOUR_PLAYER_STARVED_PROD_TARGET
            or prod_gap >= 8
            or world.max_enemy_prod >= max(1, world.my_prod) * 1.45
        )
    )
    is_four_player_conversion = (
        world.is_four_player
        and FOUR_PLAYER_CONVERSION_TURN <= world.step <= FOUR_PLAYER_CONVERSION_END_TURN
        and not is_ahead
        and four_player_conversion_profile(world)
        and (
            is_four_player_starved
            or world.my_prod < FOUR_PLAYER_CONVERSION_PROD_TARGET
            or prod_gap >= FOUR_PLAYER_LEADER_PRESSURE_MIN_GAP
        )
    )
    ...
    return {
        ...
        "is_four_player_starved": is_four_player_starved,
        "is_four_player_conversion": is_four_player_conversion,
...
def four_player_conversion_profile(world):
    if not world.is_four_player or not world.my_planets:
        return False

    initial_planets = list(world.initial_by_id.values())
    initial_my = [planet for planet in initial_planets if planet.owner == world.player]
    initial_enemy = [
        planet for planet in initial_planets if planet.owner not in (-1, world.player)
    ]
    initial_neutral = [
        planet for planet in initial_planets if planet.owner == -1
    ]
    if not initial_my or not initial_enemy:
        initial_my = world.my_planets
        initial_enemy = world.enemy_planets
        initial_neutral = world.neutral_planets
    source_planets = initial_my or world.my_planets
    enemy_planets = initial_enemy or world.enemy_planets
    neutral_planets = initial_neutral or world.neutral_planets

    high_prod_neutrals = [
        planet
        for planet in neutral_planets
        if planet.id not in world.comet_ids and planet.production >= 4
    ]
    enemy_distance = min(
        (
            nearest_distance_to_set(planet.x, planet.y, enemy_planets)
            for planet in source_planets
        ),
        default=10**9,
    )
    if not high_prod_neutrals:
        return True

    cheap_high_exists = any(
        int(planet.production) >= 4
        and int(planet.ships) <= 8
        and nearest_distance_to_set(planet.x, planet.y, source_planets) <= 24
        for planet in high_prod_neutrals
    )
    return cheap_high_exists and enemy_distance >= 60
...
def target_value(target, arrival_turns, mission, world, modes, policy):
    ...
    if target.owner == -1:
        ...
        if world.is_four_player and modes.get("is_four_player_conversion"):
            if target.production <= 2:
                value *= FOUR_PLAYER_CONVERSION_LOW_NEUTRAL_MULT
            elif target.production == 3 and not world.is_static(target.id):
                value *= FOUR_PLAYER_CONVERSION_MID_NEUTRAL_MULT
    ...
    if target.owner not in (-1, world.player) and world._weakest_enemy is not None:
        if target.owner == world._weakest_enemy:
            if not (
                world.is_four_player
                and (modes.get("is_four_player_leader_pressure") or modes.get("is_four_player_conversion"))
    ...
    if world.is_four_player and target.owner not in (-1, world.player):
        if target.owner == world._leader_enemy:
            ...
            if modes.get("is_four_player_conversion"):
                value *= FOUR_PLAYER_CONVERSION_LEADER_MULT
        ...
        if modes.get("is_four_player_conversion"):
            if target.production >= 3:
                value *= FOUR_PLAYER_CONVERSION_ENEMY_MULT
            elif target.production <= 1 and not modes.get("is_four_player_starved"):
                value *= 0.9
            if world._leader_enemy is not None and target.owner != world._leader_enemy:
                value *= 0.84
    ...
    if world.is_four_player and modes.get("is_four_player_conversion"):
        if target.owner not in (-1, world.player):
            packet_floor = min(
                ...
                max(
                    base_needed,
                    FOUR_PLAYER_PRESSURE_PACKET_MIN,
                    int(src_available * FOUR_PLAYER_PRESSURE_PACKET_FRACTION),
                ),
            )
            if target.production >= 3 or target.owner == world._leader_enemy:
                send = max(send, packet_floor)
            margin += max(0, target.production - 1)
        elif target.owner == -1 and target.production <= 2:
            margin = max(0, margin - 3)
...
def apply_score_modifiers(base_score, target, mission, world):
    ...
    if target.owner not in (-1, world.player):
        ...
        if world.is_four_player and target.production >= 3 and world.step >= FOUR_PLAYER_CONVERSION_TURN:
            score *= 1.16
            if target.owner == world._leader_enemy:
                score *= 1.18
    elif (
        ...
        and world.step >= FOUR_PLAYER_CONVERSION_TURN
        and world.my_prod < FOUR_PLAYER_CONVERSION_PROD_TARGET
    ):
        if target.production <= 2:
            score *= 0.55
        elif target.production == 3 and not world.is_static(target.id):
    ...
    if target.owner not in (-1, world.player) and world._weakest_enemy == target.owner:
        ...
        if not (
            ...
            and world.step >= 35
            and (
                leader_gap >= FOUR_PLAYER_LEADER_PRESSURE_MIN_GAP
                or world.my_prod < FOUR_PLAYER_CONVERSION_PROD_TARGET
            )
        ):
            score *= 1.2
...
def plan_moves(world, deadline=None):
    ...
    # === 4P LEADER CONVERSION PHASE ===

    if (
        world.is_four_player
        and modes.get("is_four_player_conversion")
        ...
        leader_targets = [
            ...
            if planet.owner == world._leader_enemy
        ]
        candidate_pressure_targets = leader_targets or world.enemy_planets
        pressure_targets = sorted(
            [
                planet
                for planet in candidate_pressure_targets
                if (
                    planet.production >= 2
                    or planet.owner == world._leader_enemy
                    or planet.id in world.exposed_planet_ids
                )
            ],
            key=lambda planet: (
                planet.owner != world._leader_enemy,
        ...
        for src in sorted(world.my_planets, key=lambda planet: -source_attack_left(planet.id)):
            ...
            best = None
            for target in pressure_targets[:8]:
                if expired():
                    return finalize_moves()
                send_guess = min(
                    src_left,
                    max(
                        int(target.ships) + int(target.production) * 3 + 4,
                        FOUR_PLAYER_PRESSURE_PACKET_MIN,
                        int(src_left * FOUR_PLAYER_PRESSURE_PACKET_FRACTION),
                    ),
                )
                plan = settle_plan(
                    src,
    ...
    if (
        ...
        if safe_fronts:
            ...
            if world.is_four_player and modes.get("is_four_player_starved"):
                send_ratio = max(send_ratio, FOUR_PLAYER_FUNNEL_STARVED_RATIO)
            elif world.is_four_player and modes.get("is_four_player_conversion"):
                send_ratio = max(send_ratio, FOUR_PLAYER_FUNNEL_CONVERSION_RATIO)
            ...
            if world.is_four_player and modes.get("is_four_player_conversion"):
                rear_source_min = 8
                rear_send_min = 5
                rear_distance_ratio = 1.05
                rear_stage_progress = 0.92
                rear_max_turns = 56
