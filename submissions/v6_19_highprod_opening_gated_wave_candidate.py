...
def opening_high_prod_packet_floor(target, base_needed):
    return max(
        OPENING_HIGH_PROD_MIN_PACKET,
        int(base_needed) + int(target.production) * OPENING_HIGH_PROD_PACKET_PROD_WEIGHT,
    )
...
def opening_filter(target, arrival_turns, needed, src_available, world, policy, src=None):
    ...
    if (
        ...
        and world.step < OPENING_HIGH_PROD_PACKET_TURN
        and target.production <= OPENING_HIGH_PROD_LOW_TARGET_MAX_PROD
    ):
        future_budget = max(1, int(src.ships)) + int(src.production) * OPENING_HIGH_PROD_FUTURE_TURNS
        for candidate in world.neutral_planets:
            if candidate.id == target.id or candidate.id in world.comet_ids:
                continue
            if candidate.production < OPENING_HIGH_PROD_MIN_PROD:
                continue
            need_hint = int(candidate.ships) + 1
            packet_floor = opening_high_prod_packet_floor(candidate, need_hint)
            ...
            if seeded is None:
                continue
            _, aim = seeded
            if aim[1] > OPENING_HIGH_PROD_MAX_ROUTE_TURNS:
                continue
            my_t, enemy_t = policy_reaction_times(candidate.id, policy)
            if enemy_t + 6 < my_t:
                continue
...
def preferred_send(target, base_needed, arrival_turns, src_available, world, modes, policy):
    ...
    if (
        ...
        and target.production >= OPENING_HIGH_PROD_MIN_PROD
        and src_available >= OPENING_HIGH_PROD_MIN_PACKET
    ):
        preferred = max(preferred, opening_high_prod_packet_floor(target, base_needed))
