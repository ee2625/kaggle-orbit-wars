...
FOUR_PLAYER_JOINT_SOURCE_MIN_SEND = 10
FOUR_PLAYER_FOLLOWUP_MIN_AFTER_PACKET_TURN = 14
...
def adaptive_followup_min(world, modes=None):
    floor = strategy_profile(world)["followup_min"]
    if world.is_four_player and world.step >= FOUR_PLAYER_PACKET_AGGREGATION_TURN and not world.is_late:
        floor = max(floor, FOUR_PLAYER_FOLLOWUP_MIN_AFTER_PACKET_TURN)
    return floor
...
def plan_moves(world, deadline=None):
    ...
    for mission in missions:
        ...
        if world.is_four_player and world.step >= FOUR_PLAYER_PACKET_AGGREGATION_TURN and not world.is_late:
            ...
            remaining = max(missing, FOUR_PLAYER_PACKET_JOINT_FLOOR)
            sends = {}
            for option, limit in ordered:
                if remaining <= 0:
                    break
                min_send = min(FOUR_PLAYER_JOINT_SOURCE_MIN_SEND, remaining)
                if limit < min_send:
                    continue
