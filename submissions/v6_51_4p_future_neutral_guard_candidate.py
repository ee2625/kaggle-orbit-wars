...
FUTURE_NEUTRAL_GUARD_ENABLED = True
FUTURE_NEUTRAL_GUARD_TURN_LIMIT = 85
FUTURE_NEUTRAL_GUARD_ENEMY_HORIZON = 60
FUTURE_NEUTRAL_GUARD_MIN_PROD = 3
FUTURE_NEUTRAL_GUARD_SOURCE_TOP_K = 4
FUTURE_NEUTRAL_GUARD_VALUE_MULT = 1.55
...
def build_future_neutral_guard_missions(world, policy, planned_commitments, modes):
    if (
        not FUTURE_NEUTRAL_GUARD_ENABLED
        or not world.is_four_player
        or world.step > FUTURE_NEUTRAL_GUARD_TURN_LIMIT
    ):
        return []

    missions = []
    for target in world.neutral_planets:
        if target.id in world.comet_ids or target.production < FUTURE_NEUTRAL_GUARD_MIN_PROD:
            continue
        ...
        if not friendly_etas:
            continue

        first_friendly = min(friendly_etas)
        enemy_etas = sorted({
            int(math.ceil(eta))
            for eta, owner, ships in arrivals
            if owner not in (-1, world.player)
            and ships > 0
            and first_friendly < int(math.ceil(eta)) <= FUTURE_NEUTRAL_GUARD_ENEMY_HORIZON
        })
        if not enemy_etas:
            continue

        for enemy_eta in enemy_etas[:2]:
            before_owner, _ = world.projected_state(
                target.id,
                max(1, enemy_eta - 1),
                planned_commitments=planned_commitments,
            )
            after_owner, _ = world.projected_state(
                target.id,
                enemy_eta,
                planned_commitments=planned_commitments,
            )
            if before_owner != world.player or after_owner == world.player:
                continue

            best = None
            sources = nearest_sources_to_target(
                target,
                world.my_planets,
                FUTURE_NEUTRAL_GUARD_SOURCE_TOP_K,
            )
            for src in sources:
                src_available = policy["attack_budget"].get(src.id, 0)
                if src_available < adaptive_partial_source_min(world, modes):
                ...
                seeded = world.best_probe_aim(
                    ...
                    target.id,
                    src_available,
                    hints=(target.production + 4, int(target.ships) + 1, 14),
                    max_turn=enemy_eta,
                )
                if seeded is None:
                    continue
                ...
                plan = settle_plan(
                    ...
                    policy,
                    mission="rescue",
                    eval_turn_fn=lambda _turns, enemy_eta=enemy_eta: enemy_eta,
                    anchor_turn=enemy_eta,
                )
                if plan is None:
                    continue
                ...
                angle, turns, _, need, send_pref = plan
                if turns > enemy_eta or send_pref < need:
                    continue

                saved_turns = max(1, world.remaining_steps - enemy_eta)
                value = (
                    target.production * saved_turns
                    + max(0, int(target.ships)) * DEFENSE_SHIP_VALUE
                ) * FUTURE_NEUTRAL_GUARD_VALUE_MULT
                score = value / (send_pref + turns * DEFENSE_COST_TURN_WEIGHT + 1.0)
...
def plan_moves(world, deadline=None):
    ...
    missions.extend(build_future_neutral_guard_missions(world, policy, planned_commitments, modes))
