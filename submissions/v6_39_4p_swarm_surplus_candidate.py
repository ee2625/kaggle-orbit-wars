...
FOUR_PLAYER_SWARM_SURPLUS_TURN = 58
FOUR_PLAYER_SWARM_SURPLUS_MIN_PROD = 3
FOUR_PLAYER_SWARM_SURPLUS_MIN_MY_PROD = 26
FOUR_PLAYER_SWARM_SURPLUS_RATIO = 1.28
FOUR_PLAYER_SWARM_SURPLUS_BASE = 5
FOUR_PLAYER_SWARM_SURPLUS_PROD_MULT = 3
FOUR_PLAYER_SWARM_SURPLUS_MAX_EXTRA = 34
FOUR_PLAYER_SWARM_SURPLUS_CAP_RATIO = 0.74
...
def plan_moves(world, deadline=None):
    ...
    for mission in missions:
        ...
        if (
            world.is_four_player
            and world.step >= FOUR_PLAYER_SWARM_SURPLUS_TURN
            and world.my_prod >= FOUR_PLAYER_SWARM_SURPLUS_MIN_MY_PROD
            and target.owner != -1
            and target.owner != world.player
            and target.production >= FOUR_PLAYER_SWARM_SURPLUS_MIN_PROD
        ):
            extra = min(
                FOUR_PLAYER_SWARM_SURPLUS_MAX_EXTRA,
                FOUR_PLAYER_SWARM_SURPLUS_BASE + target.production * FOUR_PLAYER_SWARM_SURPLUS_PROD_MULT,
            )
            surplus_goal = max(
                missing,
                int(missing * FOUR_PLAYER_SWARM_SURPLUS_RATIO),
                missing + extra,
            )
            surplus_cap = max(missing, int(sum(limits) * FOUR_PLAYER_SWARM_SURPLUS_CAP_RATIO))
            send_goal = min(sum(limits), surplus_goal, surplus_cap)
        if (
            world.is_four_player
            and world.step >= FOUR_PLAYER_PACKET_AGGREGATION_TURN
            and sum(limits) < max(send_goal, FOUR_PLAYER_PACKET_JOINT_FLOOR)
        ):
            continue
