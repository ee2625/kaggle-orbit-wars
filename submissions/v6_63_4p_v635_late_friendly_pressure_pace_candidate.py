...
FOUR_PLAYER_FRIENDLY_PACE_START = 85
FOUR_PLAYER_FRIENDLY_PACE_END = 140
FOUR_PLAYER_FRIENDLY_PACE_MIN_PROD = 22
FOUR_PLAYER_FRIENDLY_PACE_MIN_ENEMY_MOVES = 1
FOUR_PLAYER_FRIENDLY_PACE_CAP_BASE = 28
FOUR_PLAYER_FRIENDLY_PACE_CAP_PROD_RATIO = 1.00
FOUR_PLAYER_FRIENDLY_PACE_URGENT_HORIZON = 16
...
def four_player_friendly_pace_active(world, modes=None):
    if not world.is_four_player or world.is_late:
        return False
    if world.step < FOUR_PLAYER_FRIENDLY_PACE_START or world.step > FOUR_PLAYER_FRIENDLY_PACE_END:
        return False
    if world.my_prod < FOUR_PLAYER_FRIENDLY_PACE_MIN_PROD:
        return False
    if world.max_enemy_prod >= world.my_prod + FOUR_PLAYER_NEUTRAL_PACE_PROD_GAP_LIMIT:
        return False
    if modes is not None and modes.get("is_snowball_risk"):
...
def friendly_transfer_target(world, target):
    return (
        target is not None
...
def enemy_pressure_target(world, target):
    return target is not None and target.owner not in (-1, world.player)
...
def plan_moves(world, deadline=None):
    ...
    def finalize_moves():
        ...
        pace_friendlies = four_player_friendly_pace_active(world, modes)
        low_neutral_quota = (
            FOUR_PLAYER_NEUTRAL_PACE_EARLY_QUOTA
            if world.step < 50
            else FOUR_PLAYER_NEUTRAL_PACE_MID_QUOTA
        )
        friendly_ship_cap = max(
            FOUR_PLAYER_FRIENDLY_PACE_CAP_BASE,
            int(world.my_prod * FOUR_PLAYER_FRIENDLY_PACE_CAP_PROD_RATIO),
        )
        low_neutral_sent = 0
        friendly_ships_sent = 0
        target_cache = {}

        def target_for(source, angle, send):
            key = (source.id, round(float(angle), 6), int(send))
            if key not in target_cache:
                target_cache[key] = projected_move_target(world, source, angle, send)
            return target_cache[key]

        enemy_pressure_moves = 0
        if pace_friendlies:
            probe_used = defaultdict(int)
            for src_id, angle, ships in moves:
                source = world.planet_by_id.get(src_id)
                if source is None:
                    continue
                send = min(int(ships), int(source.ships) - probe_used[src_id])
                if send < 1:
                    continue
                if enemy_pressure_target(world, target_for(source, angle, send)):
                    enemy_pressure_moves += 1
                probe_used[src_id] += send
            if enemy_pressure_moves < FOUR_PLAYER_FRIENDLY_PACE_MIN_ENEMY_MOVES:
                pace_friendlies = False

        def urgent_friendly_target(target):
            if not friendly_transfer_target(world, target):
                return False
            status = world.hold_status(
                target.id,
                planned_commitments=planned_commitments,
                horizon=FOUR_PLAYER_FRIENDLY_PACE_URGENT_HORIZON,
            )
            return (
                not status["holds_full"]
                or (
                    status["first_enemy"] is not None
                    and status["first_enemy"] <= FOUR_PLAYER_FRIENDLY_PACE_URGENT_HORIZON
                )
            )
        ...
        for src_id, angle, ships in moves:
            ...
            if send >= 1:
                target = None
                if pace_low_neutrals or pace_friendlies:
                    target = target_for(source, angle, send)
                if pace_low_neutrals:
                    if low_value_neutral_target(world, target):
                        if low_neutral_sent >= low_neutral_quota:
                            continue
                        low_neutral_sent += 1
                if pace_friendlies and friendly_transfer_target(world, target):
                    if not urgent_friendly_target(target):
                        remaining = friendly_ship_cap - friendly_ships_sent
                        if remaining <= 0:
                            continue
                        send = min(send, remaining)
                    friendly_ships_sent += send
