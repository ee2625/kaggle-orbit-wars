...
FOUR_PLAYER_NEUTRAL_PACE_ZERO_MID_MIN_OTHER = 2
...
def plan_moves(world, deadline=None):
    ...
    def finalize_moves():
        ...
        def is_low_neutral_move(source, angle, send):
            ...
            return low_value_neutral_target(world, target)

        if pace_low_neutrals and world.step >= 50:
            ...
            for src_id, angle, ships in moves:
                ...
                if send < 1:
                    continue
                if not is_low_neutral_move(source, angle, send):
                    other_candidates += 1
                probe_used[src_id] += send
            if other_candidates >= FOUR_PLAYER_NEUTRAL_PACE_ZERO_MID_MIN_OTHER:
                low_neutral_quota = 0
        ...
        for src_id, angle, ships in moves:
            ...
            if send >= 1:
                if pace_low_neutrals:
                    if is_low_neutral_move(source, angle, send):
