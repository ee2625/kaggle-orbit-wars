...
def build_policy_state(world, deadline=None):
    ...
    for planet in world.my_planets:
        ...
        starved_economy = world.my_prod <= 10 or (
            world.my_prod <= 12 and world.my_total <= 320
        )
        if (
            starved_economy
            ...
            and (
                world.max_enemy_prod >= world.my_prod + FOUR_PLAYER_LEADER_PRESSURE_MIN_GAP
                or world.max_enemy_prod >= world.my_prod + 3
            )
            and world.max_enemy_prod >= world.my_prod * 1.35
        ):
            reserve_cap_ratio = 0.55
            if world.max_enemy_prod >= world.my_prod + 8:
                reserve_cap_ratio = 0.45
            exact_cap = max(
                int(planet.production) + 2,
                int(planet.ships * reserve_cap_ratio),
            )
            exact_keep = min(exact_keep, exact_cap)
...
def _is_endgame(world):
    """Trigger endgame refinement when the state is small and time is short.

    Restricted to 2-player games for first version — paranoid eval against a single
    opponent is well-defined. 4P endgame search is harder to model.
    """
    if world.is_four_player:
        return False
    if world.remaining_steps > 35:
        return False
    non_comet = [p for p in world.planets if p.id not in world.comet_ids]
    return len(non_comet) <= 12


def _build_synthetic_fleets(world, moves):
    new_fleets = list(world.fleets)
    next_id = max((f.id for f in world.fleets), default=-1) + 1000
    ...
    for from_id, angle, ships in moves:
        ...
        new_fleets.append(Fleet(next_id, world.player, start_x, start_y, angle, source.id, int(ships)))
        next_id += 1
    return new_fleets


def _score_endgame_position(world, moves):
    """Score a candidate move-set by forward-simulating each planet to game end.

    Optimistic in two ways: (1) assumes opponent makes no new launches in the
    remaining horizon; (2) per-planet timelines do not couple. Used as a relative
    ranker over candidates rather than an absolute value estimate.
    """
    fleets_after = _build_synthetic_fleets(world, moves)
    ...
    for planet in world.planets:
        ...
        for turn in range(1, horizon + 1):
            ...
            if owner == world.player:
                score += planet.production
            elif owner >= 0 and owner != world.player:
                score -= planet.production * 0.85
        ...
        if final_owner == world.player:
            score += final_ships * 0.25
        elif final_owner >= 0:
            score -= final_ships * 0.25
    my_in_flight = sum(f.ships for f in fleets_after if f.owner == world.player)
    enemy_in_flight = sum(
        f.ships for f in fleets_after if f.owner >= 0 and f.owner != world.player
    )
    score += my_in_flight * 0.10 - enemy_in_flight * 0.10
    return score


def _generate_endgame_variants(baseline):
    """Produce a small set of candidate move-sets to score against the baseline."""
    variants = [tuple(tuple(m) for m in baseline)]
    if baseline:
        # Defensive: do nothing this turn, hold all ships.
        variants.append(())
        # Half-commit: send half ships per move (smaller, faster fleets).
        half = []
        for from_id, angle, ships in baseline:
            ships = int(ships)
            if ships >= 4:
                half.append((from_id, angle, max(2, ships // 2)))
        if half:
            variants.append(tuple(half))
        # Drop the smallest move (reduces over-commit).
        sorted_moves = sorted(baseline, key=lambda m: int(m[2]))
        if len(sorted_moves) >= 2:
            dropped = [m for m in baseline if m is not sorted_moves[0]]
            variants.append(tuple(tuple(m) for m in dropped))
    seen = set()
    deduped = []
    for v in variants:
        key = tuple((int(m[0]), round(float(m[1]), 4), int(m[2])) for m in v)
        if key in seen:
            continue
        seen.add(key)
        deduped.append([list(m) for m in v])
    return deduped
...
def _endgame_refine(world, baseline_moves, deadline):
    ...
    for variant in candidates:
        if time.perf_counter() >= deadline:
            break
        score = _score_endgame_position(world, variant)
