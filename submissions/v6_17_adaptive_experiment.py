...
OPENING_AGGRESSION_OVERRIDE_TURN = 25
OPENING_AGGRESSION_MAX_TARGET_DISTANCE = 35.0
OPENING_AGGRESSION_MAX_ETA = 18
...
STYLE_CLASSIFY_TURN = 22
STYLE_MIN_LAUNCHES = 5
STYLE_PASSIVE_FRIENDLY_SHARE = 0.34
STYLE_NOISY_MISS_RATE = 0.30
STYLE_HEAVY_P90_SEND = 68
NOISY_SPREAD_KEEP_BASE = 3
NOISY_SPREAD_KEEP_PROD_WEIGHT = 1
HEAVY_PRESSURE_CORE_KEEP_RATIO = 0.22
HEAVY_PRESSURE_FRONTIER_KEEP_RATIO = 0.28
HEAVY_PRESSURE_MIN_KEEP = 7

PASSIVE_HARASS_ENABLED = True
PASSIVE_HARASS_START_TURN = 24
PASSIVE_HARASS_INTERVAL = 3
PASSIVE_HARASS_MAX_PER_TURN = 3
PASSIVE_HARASS_MAX_SEND = 3
PASSIVE_HARASS_MIN_SOURCE_LEFT = 4
PASSIVE_HARASS_MAX_TURNS = 30
PASSIVE_HARASS_FRONTIER_DISTANCE = 40.0
PASSIVE_HARASS_SCORE_MULT = 2.35
...
class WorldModel:
    def __init__(self, player, step, planets, fleets, initial_by_id, ang_vel, comets, comet_ids):
        ...
        self.enemy_style = "unknown"
        self.enemy_style_stats = {}
        self.shot_cache = {}
...
def opening_capture_commitment(planet, world):
    if world.step >= OPENING_AGGRESSION_OVERRIDE_TURN:
        return 0
    if int(planet.ships) <= 1:
        return 0
    ...
    for target in world.neutral_planets:
        if target.id in world.comet_ids:
            continue
        if planet_distance(planet, target) > OPENING_AGGRESSION_MAX_TARGET_DISTANCE:
            continue
        if target.production <= 0:
            continue

        hint = min(int(planet.ships), max(1, int(target.ships) + 1))
        aim = world.plan_shot(planet.id, target.id, hint)
        if aim is None:
            continue
        turns = aim[1]
        if turns > OPENING_AGGRESSION_MAX_ETA:
            continue
        ...
        need = world.min_ships_to_own_at(
            ...
            turns,
            world.player,
            upper_bound=int(planet.ships),
        )
        if need <= 0 or need > int(planet.ships):
            continue
        ...
        score = (
            ...
            - turns * 4
            - need
            - (0 if world.is_static(target.id) else 8)
        )
...
def build_policy_state(world, deadline=None):
    ...
    for planet in world.my_planets:
        ...
        enemy_style = getattr(world, "enemy_style", "unknown")
        if enemy_style == "noisy_routes" and not world.is_total_war:
            spread_keep = NOISY_SPREAD_KEEP_BASE + int(planet.production) * NOISY_SPREAD_KEEP_PROD_WEIGHT
            proactive_keep = max(proactive_keep, min(int(planet.ships), spread_keep))
        elif enemy_style == "heavy_pressure" and not world.is_total_war:
            style_keep = max(
                HEAVY_PRESSURE_MIN_KEEP,
                int(planet.ships * HEAVY_PRESSURE_CORE_KEEP_RATIO),
            )
            if (
                world.enemy_planets
                and nearest_distance_to_set(planet.x, planet.y, world.enemy_planets) < 26
            ):
                style_keep = max(style_keep, int(planet.ships * HEAVY_PRESSURE_FRONTIER_KEEP_RATIO))
            if planet.production >= 3 or style_keep > proactive_keep:
                proactive_keep = max(proactive_keep, min(int(planet.ships), style_keep))
        ...
        opening_commitment = opening_capture_commitment(planet, world)
        if opening_commitment > 0:
            max_keep_after_commitment = max(0, int(planet.ships) - opening_commitment)
            exact_keep = min(exact_keep, max_keep_after_commitment)
            proactive_keep = min(proactive_keep, max_keep_after_commitment)
...
def settle_plan(
    ...
    def evaluate(send):
        ...
        if mission in ("snipe", "crash_exploit", "harass"):
            desired = need
        elif mission == "rescue":
            desired = min(
                src_cap,
...
def build_pressure_packet_missions(world, policy, planned_commitments, modes):
    ...
    if world.step < PRESSURE_PACKET_TURN or world.remaining_steps < PRESSURE_PACKET_STOP_REMAINING:
        return []
    if world.my_prod < PRESSURE_PACKET_MIN_MY_PROD:
        return []
    ...
    for src in world.my_planets:
        ...
        ratio = PRESSURE_PACKET_BASE_RATIO
        if modes.get("is_behind") or modes.get("is_four_player_leader_pressure"):
            ratio = PRESSURE_PACKET_BEHIND_RATIO
        send_cap = min(PRESSURE_PACKET_MAX_SEND, src_available, max(PRESSURE_PACKET_MIN_SEND, int(src_available * ratio)))
        if send_cap < PRESSURE_PACKET_MIN_SEND:
            continue

        best = None
        for target in target_pool:
            desired = send_cap
            probe = world.plan_shot(src.id, target.id, desired)
            if probe is None:
                continue
            probe_turns = probe[1]
            if probe_turns > PRESSURE_PACKET_MAX_TURNS:
                continue
            need = world.min_ships_to_own_at(
                target.id,
                probe_turns,
            ...
            desired = min(desired, max(PRESSURE_PACKET_MIN_SEND, overkill_cap))
            validated = validated_pressure_shot(
                src,
                target,
                desired,
                world,
            ...
            if validated is None:
                continue
            angle, turns, need, desired = validated
...
def build_passive_harass_missions(world, policy, planned_commitments, modes):
    if not PASSIVE_HARASS_ENABLED:
        return []
    if getattr(world, "enemy_style", "unknown") != "passive_funnel":
        return []
    if world.step < PASSIVE_HARASS_START_TURN or world.is_late:
        return []
    if (world.step + world.player) % PASSIVE_HARASS_INTERVAL != 0:
        return []
    ...
    for target in world.enemy_planets:
        if target.id in world.comet_ids:
            continue
        frontier_distance = nearest_distance_to_set(target.x, target.y, world.my_planets)
        if frontier_distance > PASSIVE_HARASS_FRONTIER_DISTANCE:
            continue

        for src in nearest_sources_to_target(target, world.my_planets, 4):
            available = policy["attack_budget"].get(src.id, 0)
            if available < PASSIVE_HARASS_MIN_SOURCE_LEFT:
                continue
            src_cap = min(PASSIVE_HARASS_MAX_SEND, available)

            seeded = world.best_probe_aim(
                src.id,
                target.id,
                src_cap,
                hints=tuple(range(1, PASSIVE_HARASS_MAX_SEND + 1)),
            )
            if seeded is None:
                continue
            _, rough_aim = seeded
            rough_turns = rough_aim[1]
            if rough_turns > PASSIVE_HARASS_MAX_TURNS:
                continue
            ...
            if send > PASSIVE_HARASS_MAX_SEND or send < plan_need:
                continue
            if turns > PASSIVE_HARASS_MAX_TURNS:
                continue

            value = target_value(target, turns, "capture", world, modes, policy)
            if value <= 0:
                continue
            value *= PASSIVE_HARASS_SCORE_MULT
            score = apply_score_modifiers(
                value / (send + turns * 0.22 + 1.0),
                target,
                "harass",
                world,
            )
            score *= 1.0 + max(0.0, PASSIVE_HARASS_FRONTIER_DISTANCE - frontier_distance) / 80.0
            missions.append(Mission(
                kind="harass",
                score=score,
                target_id=target.id,
    ...
    return missions[:PASSIVE_HARASS_MAX_PER_TURN]


def validated_pressure_shot(src, target, desired, world, planned_commitments):
    tested = []
    for candidate in (
        desired,
        int(desired * 0.7),
        ...
        send = max(1, min(PRESSURE_PACKET_MAX_SEND, int(candidate)))
        if send in tested:
            continue
        tested.append(send)
        aim = world.plan_shot(src.id, target.id, send)
...
def plan_moves(world, deadline=None):
    ...
    for target_id, options in source_options_by_target.items():
        ...
        if COORDINATED_WAVE_ENABLED and len(options) >= 2:
    ...
    if allow_optional_phase():
        missions.extend(build_passive_harass_missions(world, policy, planned_commitments, modes))
    ...
    for mission in missions:
        ...
        if mission.kind == "pressure_packet":
            if pressure_packets_sent >= PRESSURE_PACKET_MAX_PER_TURN:
                continue
            ...
            if send < PRESSURE_PACKET_MIN_SEND:
                continue
            ...
            if validated is None:
                continue
            angle, turns, need, send = validated
        ...
        if mission.kind in ("single", "snipe", "rescue", "recapture", "reinforce", "crash_exploit", "harass"):
            option = mission.options[0]
            src = world.planet_by_id[option.src_id]
            ...
            elif mission.kind == "harass":
                plan = settle_plan(
                    src,
                    target,
                    min(left, PASSIVE_HARASS_MAX_SEND),
                    min(left, option.send_cap, PASSIVE_HARASS_MAX_SEND),
                    world,
                    planned_commitments,
                    modes,
                    policy,
    ...
    if (
        ...
        if safe_fronts:
            ...
            rear_source_min = 12 if world.is_four_player else REAR_SOURCE_MIN_SHIPS
            rear_send_min = 7 if world.is_four_player else REAR_SEND_MIN_SHIPS
            rear_distance_ratio = 1.12 if world.is_four_player else REAR_DISTANCE_RATIO
            rear_stage_progress = 0.86 if world.is_four_player else REAR_STAGE_PROGRESS
            rear_max_turns = 48 if world.is_four_player else REAR_MAX_TRAVEL_TURNS
...
_OPPONENT_STYLE_BY_GAME_PLAYER = {}


def _new_opponent_style_state():
    return {
        "seen_fleet_ids": set(),
        "launches": 0,
        "ships": 0,
...
def _classify_opponent_style(state, step):
    launches = max(1, state["launches"])
    p90_send = _p90(state["send_sizes"])
    friendly_share = state["friendly"] / launches
    miss_rate = state["miss"] / launches
    attack_share = state["attack"] / launches

    stats = {
        "launches": state["launches"],
        "ships": state["ships"],
        "p90_send": p90_send,
        "friendly_share": friendly_share,
        "miss_rate": miss_rate,
        "attack_share": attack_share,
    }

    if step < STYLE_CLASSIFY_TURN or state["launches"] < STYLE_MIN_LAUNCHES:
        return "unknown", stats
    if friendly_share >= STYLE_PASSIVE_FRIENDLY_SHARE:
        return "passive_funnel", stats
    if miss_rate >= STYLE_NOISY_MISS_RATE:
        return "noisy_routes", stats
    if p90_send >= STYLE_HEAVY_P90_SEND and attack_share >= 0.45:
        return "heavy_pressure", stats
    return "balanced", stats


def _update_opponent_style_memory(obs, world):
    key = _turn_state_key(obs, world.player, world.ang_vel)
    state = _OPPONENT_STYLE_BY_GAME_PLAYER.get(key)
    if state is None or world.step <= 1 or state.get("last_step", -1) > world.step + 5:
        state = _new_opponent_style_state()
        _OPPONENT_STYLE_BY_GAME_PLAYER[key] = state

    seen = state["seen_fleet_ids"]
    ...
    state["last_step"] = world.step
    style, stats = _classify_opponent_style(state, world.step)
    state["style"] = style
    return style, stats
...
def build_world(obs, inferred_step=None):
    ...
    world.enemy_style, world.enemy_style_stats = _update_opponent_style_memory(obs, world)
    return world
