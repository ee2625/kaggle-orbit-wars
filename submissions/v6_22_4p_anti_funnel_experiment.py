...
PRESSURE_PACKET_TURN = 72
PRESSURE_PACKET_STOP_REMAINING = 90
PRESSURE_PACKET_MIN_LEFT = 54
PRESSURE_PACKET_MIN_SEND = 42
PRESSURE_PACKET_MAX_SEND = 260
PRESSURE_PACKET_BASE_RATIO = 0.58
PRESSURE_PACKET_BEHIND_RATIO = 0.76
PRESSURE_PACKET_MAX_TURNS = 36
...
PRESSURE_PACKET_MAX_PER_TURN = 2
PRESSURE_PACKET_MIN_MY_PROD = 28

ANTI_FUNNEL_ENABLED = True
ANTI_FUNNEL_START_TURN = 24
ANTI_FUNNEL_STOP_REMAINING = 110
ANTI_FUNNEL_MIN_ACTIVE_SHIPS = 55
ANTI_FUNNEL_MIN_ACTIVE_COUNT = 4
ANTI_FUNNEL_FRIENDLY_SHARE = 0.34
ANTI_FUNNEL_CONTROLLED_PROD = 42
ANTI_FUNNEL_CONTROLLED_FRIENDLY_SHARE = 0.25
ANTI_FUNNEL_MIN_PROD_GAP = 3
ANTI_FUNNEL_MIN_PROD_RATIO = 1.08
ANTI_FUNNEL_MAX_TURNS = 34
ANTI_FUNNEL_MIN_SOURCE = 9
ANTI_FUNNEL_MIN_SEND = 7
ANTI_FUNNEL_MAX_SEND = 58
ANTI_FUNNEL_SEND_RATIO = 0.54
ANTI_FUNNEL_VALUE_MULT = 1.42
ANTI_FUNNEL_SCORE_MULT = 1.22
ANTI_FUNNEL_RECEIVING_SHIP_BONUS = 0.015
ANTI_FUNNEL_EXPOSED_BONUS = 1.22
ANTI_FUNNEL_REAR_SEND_RATIO = 0.76
...
def analyze_enemy_funnel_styles(player, step, planets, fleets, initial_by_id, ang_vel, comets, comet_ids):
    stats = {}
    for fleet in fleets:
        ...
        row = stats.setdefault(
            owner,
            {
                ...
                "neutral_ships": 0,
                "miss_ships": 0,
                "receiving_by_target": defaultdict(int),
        ...
        row["outgoing_by_source"][int(fleet.from_planet_id)] += ships

        target, _eta = fleet_target_planet_dynamic(
            fleet,
            planets,
            initial_by_id,
            ang_vel,
            comets,
            comet_ids,
            max_turns=ANTI_FUNNEL_MAX_TURNS + 18,
        )
        if target is None:
            row["miss_ships"] += ships
        elif target.owner == owner:
            row["friendly_ships"] += ships
            row["receiving_by_target"][target.id] += ships
        elif target.owner == -1:
            row["neutral_ships"] += ships
        else:
            row["enemy_ships"] += ships

    funnel_owner_ids = set()
    controlled_owner_ids = set()
    receiving_by_target = defaultdict(int)

    for owner, row in stats.items():
        ships = max(1, row["ships"])
        row["friendly_share"] = row["friendly_ships"] / ships
        row["enemy_share"] = row["enemy_ships"] / ships
        row["miss_share"] = row["miss_ships"] / ships
        row["p90_send"] = percentile_int(row["sizes"], 0.90)
        owner_prod = sum(int(planet.production) for planet in planets if planet.owner == owner)
        row["production"] = owner_prod

        active_enough = (
            row["ships"] >= ANTI_FUNNEL_MIN_ACTIVE_SHIPS
            or row["count"] >= ANTI_FUNNEL_MIN_ACTIVE_COUNT
        )
        passive_funnel = (
            step >= ANTI_FUNNEL_START_TURN
            and active_enough
            and row["friendly_share"] >= ANTI_FUNNEL_FRIENDLY_SHARE
        )
        controlled_funnel = (
            step >= ANTI_FUNNEL_START_TURN
            and active_enough
            and owner_prod >= ANTI_FUNNEL_CONTROLLED_PROD
            and row["friendly_share"] >= ANTI_FUNNEL_CONTROLLED_FRIENDLY_SHARE
        )
        if passive_funnel or controlled_funnel:
            funnel_owner_ids.add(owner)
        if controlled_funnel:
            controlled_owner_ids.add(owner)
        if owner in funnel_owner_ids:
            for target_id, target_ships in row["receiving_by_target"].items():
                receiving_by_target[target_id] += target_ships

    return {
        "owners": stats,
        "funnel_owner_ids": funnel_owner_ids,
        "controlled_owner_ids": controlled_owner_ids,
        "receiving_by_target": dict(receiving_by_target),
    }
...
class WorldModel:
    def __init__(self, player, step, planets, fleets, initial_by_id, ang_vel, comets, comet_ids):
        ...
        self.enemy_funnel = analyze_enemy_funnel_styles(
            player, step, planets, fleets, initial_by_id, ang_vel, comets, comet_ids,
        )
        self.funnel_enemy_ids = set(self.enemy_funnel["funnel_owner_ids"])
        self.controlled_funnel_enemy_ids = set(self.enemy_funnel["controlled_owner_ids"])
        self.enemy_funnel_receiving_by_target = self.enemy_funnel["receiving_by_target"]
        self.shot_cache = {}
...
def build_modes(world):
    ...
    funnel_enemy_prod = max(
        (world.owner_production.get(owner, 0) for owner in world.funnel_enemy_ids),
        default=0,
    )
    funnel_is_prod_threat = (
        funnel_enemy_prod >= world.my_prod + ANTI_FUNNEL_MIN_PROD_GAP
        or funnel_enemy_prod >= world.my_prod * ANTI_FUNNEL_MIN_PROD_RATIO
        or funnel_enemy_prod >= ANTI_FUNNEL_CONTROLLED_PROD
    )
    is_enemy_funneling = (
        ANTI_FUNNEL_ENABLED
        and world.is_four_player
        and world.funnel_enemy_ids
        and funnel_is_prod_threat
        and world.step >= ANTI_FUNNEL_START_TURN
        and world.remaining_steps >= ANTI_FUNNEL_STOP_REMAINING
    )
    is_controlled_funnel_risk = bool(world.controlled_funnel_enemy_ids) and is_enemy_funneling
    ...
    return {
        ...
        "is_enemy_funneling": is_enemy_funneling,
        "is_controlled_funnel_risk": is_controlled_funnel_risk,
        "funnel_enemy_prod": funnel_enemy_prod,
...
def target_value(target, arrival_turns, mission, world, modes, policy):
    ...
    elif mission == "anti_funnel":
        value *= ANTI_FUNNEL_VALUE_MULT

    if target.id in world.exposed_planet_ids:
        value *= EXPOSED_PLANET_VALUE_MULT
    if (
        target.owner in world.funnel_enemy_ids
        and target.owner not in (-1, world.player)
        and modes.get("is_enemy_funneling")
    ):
        value *= 1.12
        if target.owner in world.controlled_funnel_enemy_ids:
            value *= 1.08
...
def apply_score_modifiers(base_score, target, mission, world):
    ...
    if mission == "snipe":
        ...
        score *= ANTI_FUNNEL_SCORE_MULT
    if target.id in world.exposed_planet_ids:
        score *= 1.25
    if (
        world.is_four_player
        and target.owner in world.funnel_enemy_ids
        and target.owner not in (-1, world.player)
    ):
        score *= 1.08
    if target.owner not in (-1, world.player):
...
def build_pressure_packet_missions(world, policy, planned_commitments, modes):
    if not PRESSURE_PACKET_ENABLED:
        return []
    profile = strategy_profile(world)
    if world.step < PRESSURE_PACKET_TURN or world.remaining_steps < PRESSURE_PACKET_STOP_REMAINING:
        return []
    if world.my_prod < profile["pressure_packet_min_my_prod"]:
        return []
    ...
    for src in world.my_planets:
        src_available = policy["attack_budget"].get(src.id, 0)
        if src_available < PRESSURE_PACKET_MIN_LEFT:
            continue
        ...
        send_cap = min(
            ...
            src_available,
            max(profile["pressure_packet_min_send"], int(src_available * ratio)),
        )
        if send_cap < profile["pressure_packet_min_send"]:
            continue
        ...
        for target in target_pool:
            ...
            if aim is None:
                continue
            angle, turns, _, _ = aim
            if turns > PRESSURE_PACKET_MAX_TURNS:
                continue
            if not candidate_time_valid(target, turns, world, LATE_CAPTURE_BUFFER):
                continue
            ...
            desired = min(desired, max(profile["pressure_packet_min_send"], overkill_cap))
            if desired < need:
                continue
            if desired != send_cap:
            ...
            if desired >= 64 and not shot_hits_intended_target(
                ...
                world.comets,
                world.comet_ids,
                max_turns=min(PRESSURE_PACKET_MAX_TURNS + 2, turns + 3),
            ):
                continue
...
def build_anti_funnel_missions(world, policy, planned_commitments, modes):
    if not modes.get("is_enemy_funneling"):
        return []
    if not world.enemy_planets:
        return []
    ...
    target_pool = []
    for target in world.enemy_planets:
        if target.owner not in world.funnel_enemy_ids:
            continue
        if target.id in world.comet_ids:
            continue
        receiving = world.enemy_funnel_receiving_by_target.get(target.id, 0)
        exposed = target.id in world.exposed_planet_ids
        if target.production <= 1 and receiving < 10 and not exposed:
            continue
    ...
    for target in sorted(
        target_pool,
        key=lambda planet: (
            0 if planet.id in world.exposed_planet_ids else 1,
            -world.enemy_funnel_receiving_by_target.get(planet.id, 0),
            -int(planet.production),
            nearest_distance_to_set(planet.x, planet.y, world.my_planets),
        ),
    )[:8]:
        receiving = world.enemy_funnel_receiving_by_target.get(target.id, 0)
        source_pool = sorted(
            world.my_planets,
            key=lambda source: planet_distance(source, target),
        )[:5]
        for src in source_pool:
            src_available = policy["attack_budget"].get(src.id, 0)
            if src_available < ANTI_FUNNEL_MIN_SOURCE:
                continue

            source_cap = min(src_available, ANTI_FUNNEL_MAX_SEND)
            seeded = world.best_probe_aim(
                src.id,
                target.id,
                source_cap,
                hints=(int(target.ships) + 1, int(target.ships) + 4, source_cap),
                max_turn=ANTI_FUNNEL_MAX_TURNS,
            )
            if seeded is None:
                continue
            _probe_ships, rough_aim = seeded
            ...
            need = world.min_ships_to_own_at(
                ...
                world.player,
                planned_commitments=planned_commitments,
                upper_bound=source_cap,
            )
            if need <= 0 or need > source_cap:
                continue

            send_guess = min(
                source_cap,
                max(
                    ANTI_FUNNEL_MIN_SEND,
                    int(src_available * ANTI_FUNNEL_SEND_RATIO),
                    need + (1 if target.id in world.exposed_planet_ids else 3),
                ),
            )
            plan = settle_plan(
                src,
            ...
            angle, turns, _, plan_need, send_pref = plan
            if turns > ANTI_FUNNEL_MAX_TURNS:
                continue
            if send_pref < plan_need:
                continue
            if not candidate_time_valid(target, turns, world, LATE_CAPTURE_BUFFER):
                continue

            value = target_value(target, turns, "anti_funnel", world, modes, policy)
            if value <= 0:
                continue
            value *= 1.0 + min(0.45, receiving * ANTI_FUNNEL_RECEIVING_SHIP_BONUS)
            if target.id in world.exposed_planet_ids:
                value *= ANTI_FUNNEL_EXPOSED_BONUS

            score = apply_score_modifiers(
                value / (send_pref + turns * 0.36 + 1.0),
                target,
                "anti_funnel",
                world,
            )
...
def plan_moves(world, deadline=None):
    ...
    if allow_heavy_phase():
        missions.extend(build_anti_funnel_missions(world, policy, planned_commitments, modes))
    ...
    for mission in missions:
        ...
        if mission.kind == "pressure_packet":
            if pressure_packets_sent >= profile["pressure_packet_max_per_turn"]:
                continue
            ...
            if send < profile["pressure_packet_min_send"]:
                continue
        ...
        if mission.kind in ("single", "snipe", "rescue", "recapture", "reinforce", "crash_exploit", "anti_funnel"):
            option = mission.options[0]
            src = world.planet_by_id[option.src_id]
    ...
    if (
        ...
        if safe_fronts:
            ...
            if modes.get("is_enemy_funneling"):
                send_ratio = max(send_ratio, ANTI_FUNNEL_REAR_SEND_RATIO)
